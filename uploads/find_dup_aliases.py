"""Find gpkg stations within 5km of any curated RU station that weren't
caught by name-dedup. These are likely the same station under a different
name (e.g. Adler vs Sochi)."""
...
print("Within 5km of curated but not caught by name dedup (likely aliases):")
hits = []
for r in stations:
    ...
    # check proximity
    for hid, hen, hru, hlat, hlng in CURATED:
        d = hv(r["lat"], r["lng"], hlat, hlng)
        if d < 5.0:
            hits.append((r, hid, hen, d))
            break
for r, hid, hen, d in hits:
    print(f"  fid={r['fid']} en='{r['name_en']}' ru='{r['name_ru']}' [{r['lat']:.3f},{r['lng']:.3f}] -> curated {hid} ({hen}) {d:.2f}km")
